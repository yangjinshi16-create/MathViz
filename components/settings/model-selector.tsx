...
export function ModelSelector({
  ...
  // Derive active provider from the controlled providerId prop
  const effectiveProvider = configuredProviders.some((p) => p.id === providerId)
    ? providerId
    ...
    async (pid: ProviderId, mid: string) => {
      ...
      try {
        ...
        try {
          ...
          setTestStatus('error');
          setTestMessage(t('settings.connectionFailed') + ': Invalid server response');
          return;
        }
        ...
        if (data.success) {
          ...
          setTestStatus('error');
          setTestMessage(data.error || data.details || t('settings.connectionFailed'));
        }
      } catch {
        setTestStatus('error');
          ...
          {configuredProviders.map((provider) => {
            ...
            return (
              <button
                key={provider.id}
                onClick={() => {
                  // Switch to this provider; keep current model if it belongs to this provider
                  const firstModel = providersConfig[provider.id]?.models?.[0]?.id || '';
                  if (provider.id === providerId) {
                    // Already selected provider, no-op
                  } else {
                    onModelChange(provider.id, firstModel);
                  }
                }}
                className={cn(
